...
def _dedupe_languages(summaries: list[dict], lang: str = "de") -> list[dict]:
    """Collapse multi-language variants of the same publication to one entry.

    The portal publishes the same notice once per language; the variants share
    a `publicationNumber` but differ in `id`. Without this, a bilingual canton
    like TI returns every hit twice and the model reports inflated counts.
    The variant matching `lang` wins; otherwise the first one seen is kept.
    """
    chosen: dict[str, dict] = {}
    order: list[str] = []
    for s in summaries:
        key = s.get("publicationNumber") or s.get("id")
        if key is None:
            continue
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "`search_publications` or `search_procurement`, then `get_publication(id=…)` "
...
class PublicationInput(BaseModel):
    ...
    id: str = Field(
        ...
        description=(
            ...
            "`search_procurement`. Beispiel: 'fbf0ff9e-3e28-4e09-8a1e-32a7aa4cea8f'."
        ),
        min_length=8,
        max_length=64,
    )
...
async def search_publications(params: SearchInput) -> str:
    ...
    Returns:
        str: Trefferliste mit Datum, Kanton, Rubrik, Titel, ID und Quell-URL.
    """
    ...
    total = data.get("total")
    summaries = _dedupe_languages(
        [_meta_summary(i, params.language) for i in content], params.language
    )
...
@logged_tool("search_procurement")
async def search_procurement(params: ProcurementInput) -> str:
    ...
    Args:
        params (ProcurementInput):
            ...
            - limit (int): 1–100 (Standard 20), page (int): 0-basiert
            - language (str), response_format (str)

    Returns:
        str: Ausschreibungen (neueste zuerst) mit Datum, Kanton, Titel, ID.
    """
...
async def source_status(params: StatusInput) -> str:
